    ...
    internal static class UseCaseTestHelper
    {
        ...
        private static void AssertWhenTypesDiffer(Exception useCaseException, Exception conditionsException)
        {
            ...
            if (useCaseException != null && conditionsException != null)
            {
                ...
                Assert.True(false, string.Format("Both cases throw a different type of exception. " +
            ...
            {
                Assert.True(false, "The use case action didn't throw an exception, while the conditions action did.");
            }
            else
            {
                Assert.True(false, "The use case action threw an exception, while the conditions action didn't.");
            }
        }
